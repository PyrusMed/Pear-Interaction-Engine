		...
		*/
		...
		IEnumerator proximityWatcher()
		{
			...
			while (true)
			{
				...
				if (_currentObj != null)
				{
					Collider[] objsInRange = Physics.OverlapSphere(transform.position, OffDistance, Layer);
					bool currentObjOutOfRange = true;
					foreach (Collider nearbyObj in objsInRange)
					{
						if (nearbyObj.gameObject == _currentObj)
						{
							currentObjOutOfRange = false;
							break;
						}
					}

					if (currentObjOutOfRange)
					{
						_currentObj = null;
